...
import { getArtifactConfig } from "@/lib/types/artifact-types";
    ...
    <section>
      <div className="mb-4 flex flex-wrap items-baseline justify-between gap-3">
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
          <h2 className="text-lg font-bold">クエスト</h2>
          <p className="text-sm text-gray-600">
            全 {missions.length} 件（公開中 {visible.length}）
          </p>
        </div>
      ...
      {visibleWithoutCategory.length > 0 && (
        <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4">
          <p className="text-sm font-bold text-amber-900">
            カテゴリに入っていない公開クエストが {visibleWithoutCategory.length}{" "}
            件あります
          </p>
