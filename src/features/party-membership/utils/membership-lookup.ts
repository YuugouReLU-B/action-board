...
export function buildMembershipLookupResponse(
  ...
  if (!userExists || !userId) {
    return {
      ...
      notes: [
        "このメールアドレスで登録された浜通りクエストユーザーが見つかりません。",
        "党員バッジを表示するには、浜通りクエストのログイン用メールアドレスと党員登録のメールアドレスが一致している必要があります。",
  ...
  if (!membership) {
    return {
      ...
      notes: [
        ...
        "考えられる原因: (1) 党員登録のメールアドレスが浜通りクエストのログイン用メールアドレスと異なる、(2) 寄付（donation）のみでプラン対象外、(3) 日次同期（毎日0:30 JST実行）がまだ実行されていない。",
      ],
    };
  }
