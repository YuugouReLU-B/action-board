-- mission_category_view に event_type/tag1-3/event_end_date/supplement を追加する。
--
-- 一覧はこのビュー経由で MissionForComponent (Tables<"missions"> 相当) に
-- 変換されるため、missions に列を足したらここにも通す必要がある。
...
-- SECURITY INVOKER にしておかないと、ビュー所有者の権限で実行され RLS を迂回する
ALTER VIEW mission_category_view SET (security_invoker = true);

GRANT SELECT ON mission_category_view TO anon, authenticated, service_role;
